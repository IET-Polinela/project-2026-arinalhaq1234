...
// ---------------------------------------------------------------------------
const TEST_CITIZEN_USERNAME = process.env.TEST_CITIZEN_USERNAME;
const TEST_CITIZEN_PASSWORD = process.env.TEST_CITIZEN_PASSWORD;
const TEST_ADMIN_USERNAME = process.env.TEST_ADMIN_USERNAME;
const TEST_ADMIN_PASSWORD = process.env.TEST_ADMIN_PASSWORD;

if (
    !TEST_CITIZEN_USERNAME ||
    !TEST_CITIZEN_PASSWORD ||
    !TEST_ADMIN_USERNAME ||
    !TEST_ADMIN_PASSWORD
) {
    throw new Error(
        'Kredensial E2E belum tersedia. Salin .env.example menjadi .env, ' +
        'lalu isi akun test lokal sebelum menjalankan Playwright.'
    );
}
 ...
 *
 * Menggunakan wildcard API route, fungsi ini akan mencegat request ke domain apapun
...
async function mockSPAApiUrl(page) {
    ...
    // Gunakan wildcard API route untuk menangkap dari host/domain mana saja
...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-06: Kedua token kadaluarsa → localStorage dibersihkan, redirect ke #login', async ({ page }) => {
        ...
        expect([null, TEST_CITIZEN_USERNAME]).toContain(usernameAfter);
